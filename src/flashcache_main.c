 ...
 */
//脏数据已经写回磁盘  进行元数据更新 将脏数据的metadata从内存刷到ssd  
//保证在掉电情况下元数据可从ssd找回
void
flashcache_md_write(struct kcached_job *job)
{
	struct cache_c *dmc = job->dmc;
	//cache_md_block_head的nr_in_prog字段控制更新次序 kcached_job挂在queue_updates上
	struct cache_md_block_head *md_block_head;
	unsigned long flags;
...
flashcache_dirty_writeback(struct cache_c *dmc, int index)
{
	...
	if (unlikely(job == NULL)) {
		...
		kcopyd_copy(flashcache_kcp_client, &job->job_io_regions.cache, 1, &job->job_io_regions.disk, 0, 
			    ...
			    job);
#else
		//kcopyd服务的句柄 源 目的数量 目的地 额外标识为0 回调函数为异步 回调函数的参数
...
{	
	...
	if (pjob1 == NULL) {
		queued = -ENOMEM;//#define ENOMEM  12/* Out of memory */
		goto out;
	}
